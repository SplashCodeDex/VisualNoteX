/**
 * Optimized Drawnix Component - Performance-enhanced version with Web Worker support
 * Provides better performance for freehand drawing and large canvases
 */
...
import { withOptimizedFreehand } from './plugins/freehand/with-optimized-freehand';
...
export interface OptimizedDrawnixProps {
  value: PlaitElement[];
  viewport?: Viewport;
  theme?: PlaitTheme;
  ...
  webWorkerSmoothing?: boolean;
  adaptiveRendering?: boolean;
}

export const OptimizedDrawnix: React.FC<OptimizedDrawnixProps> = ({
  value,
  viewport,
  theme,
  onChange,
  ...
  useEffect(() => {
    if (enablePerformanceOptimizations) {
      console.log('[OptimizedDrawnix] Performance optimizations enabled:', {
        webWorkerSmoothing,
        adaptiveRendering,
        webWorkersSupported: typeof Worker !== 'undefined'
      });
    }
  }, [enablePerformanceOptimizations, webWorkerSmoothing, adaptiveRendering]);
  ...
  const plugins: PlaitPlugin[] = [
    ...
    // Use optimized freehand plugin with performance settings
    enablePerformanceOptimizations
      ? withOptimizedFreehand(board!, {
          enableOptimization: webWorkerSmoothing,
          useWebWorkers: webWorkerSmoothing,
          adaptiveRendering,
        })
      : withOptimizedFreehand(board!, { enableOptimization: false }),
      ...
      <DrawnixContext.Provider value={{ appState, setAppState }}>
        <div
          className={classNames('drawnix', {
            ...
            'drawnix--optimized': enablePerformanceOptimizations,
          })}
          ref={containerRef}
        >
          <Wrapper
...
export const Drawnix = OptimizedDrawnix;